...
// 	schema := method.GetInput().(protoc.Property)
// 	flow := FindFlow(manifest, "simple")
// 	specs := FindNode(flow, "first").Call.GetRequest().Property

// 	msg := dynamic.NewMessage(schema.GetMessageDescriptor())
...
// 			response := dynamic.NewMessage(schema.GetMessageDescriptor())
...
// 			inputAsProto := dynamic.NewMessage(schema.GetMessageDescriptor())
